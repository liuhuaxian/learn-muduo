...
void onRequest(const HttpRequest& req, HttpResponse* resp)
{
  ...
  string query(req.query(),1);
  /*auto querymap = req.queryMap();
  parseGetQuery(query,querymap);
  */

  for (const auto& myquery : req.queryMap())
  {
	  
     std::cout << myquery.first << ": " << myquery.second << std::endl;
  }
  ...
  if (req.path() == "/")
  {
    ...
    resp->setBody("<html><head><title>This is title</title></head>"
        "<body><h1>Hello</h1>Now is " + now + query + req.getqueryMap("name")+
        "</body></html>");
  }
